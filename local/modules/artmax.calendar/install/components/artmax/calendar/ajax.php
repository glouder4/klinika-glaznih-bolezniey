...
switch ($action) {
    ...
    case 'getEvent':
        ...
        try {
            ...
            // Логируем данные события перед отправкой
            file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/debug_calendar_ajax.log', 
                "AJAX_GET_EVENT: Event data before JSON encode: " . print_r($event, true) . "\n", 
                FILE_APPEND | LOCK_EX);
            
            // Проверяем права на просмотр (только автор события)
            if ($event['USER_ID'] != $GLOBALS['USER']->GetID()) {
                http_response_code(403);
                die(json_encode(['success' => false, 'error' => 'Нет прав на просмотр']));
            }
            
            $response = ['success' => true, 'event' => $event];
            file_put_contents($_SERVER['DOCUMENT_ROOT'] . '/debug_calendar_ajax.log', 
                "AJAX_GET_EVENT: Sending response: " . json_encode($response) . "\n", 
                FILE_APPEND | LOCK_EX);
    ...
    case 'updateEvent':
        ...
        if ($existingEvent['USER_ID'] != $GLOBALS['USER']->GetID()) {
            http_response_code(403);
